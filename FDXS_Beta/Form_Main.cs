...
namespace FDXS_Beta
{
    ...
    public partial class Form_Main : Form
    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            var xdata_s = en.TXiaoshou.GroupBy(r => new { date = DbFunctions.TruncateTime(r.charushijian), hour = SqlFunctions.DatePart("hh", r.charushijian) }).
                Select(r => new
                {
                    Xd = r.Key.date,
                    X = r.Key.hour,
                ...
                OrderBy(r => r.X).ToList();
            var xdata_j = en.TXiaoshou.GroupBy(r => new { date = DbFunctions.TruncateTime(r.charushijian), hour = SqlFunctions.DatePart("hh", r.charushijian) }).
                Select(r => new
                {
                    Xd = r.Key.date,
                    X = r.Key.hour,
            ...
            var wdata_s = en.TXiaoshou.GroupBy(r => DbFunctions.TruncateTime(r.charushijian)).
                Select(r => new
                {
                    wn = SqlFunctions.DateName("dw", r.Key),
                    ws = SqlFunctions.DatePart("dw", r.Key),
                    Y = r.Sum(rx => rx.shuliang)
                }).GroupBy(r => new { r.wn, r.ws }).
                Select(r => new { X = r.Key.wn, xn = r.Key.ws, Y = r.Average(rr => rr.Y) }).
                OrderBy(r => r.xn).ToList();
            var wdata_j = en.TXiaoshou.GroupBy(r => DbFunctions.TruncateTime(r.charushijian)).
                 Select(r => new
                 {
                     wn = SqlFunctions.DateName("dw", r.Key),
                     ws = SqlFunctions.DatePart("dw", r.Key),
