...
namespace JCSJGL
{
    public partial class Page_KuanhaoFenxi : MyPage
    {
        ...
        private Chart makeHotChart(TXiaoshou[] xss)
        {
            ...
            for (int pointIndex = 0; pointIndex < ws.Points.Count; pointIndex++)
            {
                ws.Points[pointIndex].Url = string.Format("Page_Xiaoshou.aspx?xsrqstart={0}&xsrqend={1}&kh={2}&ppid={3}&jmsid={4}&fdid={5}",
                    txb_xsrq_start.Text, txb_xsrq_end.Text, ws.Points[pointIndex].AxisLabel, cmb_pps.SelectedValue,cmb_jms.SelectedValue,"");
            }
        ...
        private TXiaoshou[] getXiaoshouData()
        {
            ...
            DBContext db = new DBContext();
            int rCount = 0;
            TXiaoshou[] xses = db.GetXiaoshousByCond(ppsid,fdids, "", "", xsrq_start, xsrq_end, null, null, null, null, out rCount);
        ...
        private int? getPpsid()
        {
               ...
               _LoginUser.juese == (byte)Tool.JCSJ.DBCONSTS.USER_XTJS.总经理)
            {
                ...
                {
                    return int.Parse(cmb_pps.SelectedValue);
                }
            }
            else if (_LoginUser.juese == (byte)Tool.JCSJ.DBCONSTS.USER_XTJS.品牌商管理员 ||
               _LoginUser.juese == (byte)Tool.JCSJ.DBCONSTS.USER_XTJS.品牌商经理)
        ...
        private int[] getFdids()
        {
            ...
            if (_LoginUser.juese == (byte)Tool.JCSJ.DBCONSTS.USER_XTJS.系统管理员 ||
                _LoginUser.juese == (byte)Tool.JCSJ.DBCONSTS.USER_XTJS.总经理)
            {
                if (string.IsNullOrEmpty(cmb_pps.SelectedValue) && string.IsNullOrEmpty(cmb_jms.SelectedValue))
                {
                    throw new MyException("品牌商或者加盟商必须选择一个", null);
                }
